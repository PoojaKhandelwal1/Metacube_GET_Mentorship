...
public class Poly {
	...
	**/
	public int degree( List<List<Integer>> lists ){
	    ...
	    while(i < lists.size()){
	    	  sum = 0;
	    	  for(int j = 0;j< lists.get(i).size(); j++){//spacing ?????????????
...
public static void main(String[] args) { //indentation ?????????????
	  ...
	  int n = obj.nextInt();
	  	 ...
	  	 int d = obj.nextInt();
	  while(i<n){
		  List<Integer> list = new ArrayList<Integer>(); //diff b/w List & ArrayList
		  	
		  	for(int j=0;j<d;j++){
			  System.out.println("enter power of variable "+(j+1)+" in term "+(i+1)); //messaging for power 0 ?????????????
	  ...
	  System.out.println("out of the loop"); //not correct ?????????????
	 int ans = new Poly().degree(lists);
	 System.out.println("power is "+ ans); //not correct ?????????????
