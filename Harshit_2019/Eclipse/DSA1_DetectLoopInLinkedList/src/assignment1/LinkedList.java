 ...
 //exception handling missing ?????????????
   public class LinkedList{
	 public Node head;
		...
		* @param:  (1) data- integer - data provided by user to add in the list
	  ...
	  public Node insert(int data){
		 ...
		 if(head == null){
			 head=node;
			 return head;
		 } else{ // line break, spacing, indentation ?????????????
			 Node n=head;
			 while(n.next != null){
				  n=n.next;
			 	}
	 ...
	 public void display(){
		 ...
		 System.out.println(n.data); //what if n null ?????????????
	 }
	  ...
	  /**
		...
		**/
	 public boolean containsLoop(){ //explain ?????????????
		 Node fast = head;// var name ?????????????
		 Node slow = head; // var name ?????????????
